...
:------| :----------| :-------- | :---------| :-------:
DUT    | 192.168.1.1/30 | 192.168.1.5/30 | 192.168.1.9/30 | 192.168.1.13/30
ATE    | 192.168.1.2/30 | 192.168.1.6/30 | 192.168.1.10/30 | 192.168.1.14/30

#### IPv6 Addresses
Device | Lag 1 vlan 10 | lag1 vlan 20 | lag2 vlan 10 | lag2 vlan20
:------| :----------| :-------- | :---------| :-------:
DUT    | 2002:db8::1/126 | 2002:db8::5/126 | 2002:db8::9/126 | 2002:db8::13/126
ATE    | 2002:db8::2/126 | 2002:db8::6/126 | 2002:db8::10/126 | 2002:db8::14/126
...
### RT-5.14.2 - Configure the ATE
  ...
  6. Create vlan 10 and vlan 20 on both the aggregate interfaces and configure 
      both the IPV4 PV6 addresses on vlans.
  7. Create IP flows from aggregate interface with the following mapping
      *  Flow 1: 192.168.1.2 - 192.168.1.10
      *  Flow 2: 192.168.1.6 - 192.168.1.14
      *  Flow 3: 2002:db8::2/126 - 2002:db8::10/126
      *  Flow 4: 2002:db8::6/126 - 2002:db8::14/126
  8. Push Config to the OTG.
  9. Start Protocol on the OTG

### RT-5.14.3 - Testing Steps

  * Wait for 6O seconds or until the Lags come up
  * Validate that all the 4 ports are Operationally UP on the OTG
  * Validate that the LAG is Aggregate Interfaces are UP on the OTG
  * Start Traffic flows
...
Paths:
  ## Configuration coverage
      ...
      instance	/network-instances/network-instance/config/name
      /interfaces/interface/name
      /interfaces/interface/config/name
      /interfaces/interface/config/name/lacp-mode
